...
class GeminiCorrector(
    ...
    companion object {
        ...
        fun explain(raw: String?): String {
            ...
            return when {
                ...
                "sub_daily_limit" in lower ->
                    "오늘 프리미엄 한도(2만 자)를 다 썼습니다. 내일 다시 쓸 수 있어요."
         ...
         * 짧은 글이면 1~3 초면 온다.
         *
         * 15 초였는데 늘렸다. 교정 창을 6,000 자까지 열었더니 모델이 그만큼을 **글자
         * 단위로 뱉는 시간**이 그대로 대기 시간이 된다. 15 초로는 긴 글에서 답이 오는
         * 중에 우리가 끊어 버리고, 사용자는 "긴 글은 AI 가 안 된다" 로 겪는다.
         * 대신 이 시간을 다 쓰고 실패하면 다른 모델로 옮겨 간다.
         */
