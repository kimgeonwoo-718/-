...
<p>다음 경우에만 글이 앱 서버를 거쳐 ${providerApi} 로 전송되고 결과가 돌아옵니다.</p>
<ul>
...
<ul>
<li><strong>계정 삭제(회원 탈퇴)</strong>: 앱 → 오른쪽 위 ☰ → 더보기 → <strong>회원 탈퇴</strong> 를 누르면 서버의 계정 정보(3번의 계정 항목)가 즉시 삭제됩니다.</li>
<li><strong>그 밖의 요구</strong>: 앱의 더보기 → 고객센터로 메일을 보내 주세요. 메일 밑에 붙는 설치 식별자로 해당 기기의 기록을 찾아 <strong>10일 안에</strong> 처리하고 결과를 알려 드립니다. 앱을 쓸 수 없으면 아래 문의처로 직접 보내셔도 됩니다.</li>
<li><strong>AI 전송을 멈추려면</strong>: AI 교정·번역 버튼을 누르지 않으면 글은 전송되지 않습니다. 실시간 교정만으로도 앱을 쓸 수 있습니다.</li>
</ul>
...
<h2>7. 파기</h2>
<p>보관 기간(3번)이 끝나거나 탈퇴·삭제 요청을 받으면 서버 데이터베이스에서 <strong>지체 없이, 되살릴 수 없게</strong> 지웁니다. 하루 사용량 기록은 이틀 뒤 자동으로 지워집니다. 종이로 보관하는 개인정보는 없습니다.</p>

<h2>8. 안전성 확보 조치</h2>
<ul>
<li>앱과 서버 사이의 모든 통신은 암호화(HTTPS)됩니다.</li>
<li>AI 로 보낸 글은 서버에 저장하지 않습니다.</li>
<li>구글 계정 고유 번호는 되돌릴 수 없는 값(해시)으로 바꿔 저장합니다.</li>
<li>서버의 비밀 키와 데이터베이스에는 운영자만 접근할 수 있습니다.</li>
</ul>

<h2>9. 만 14세 미만 아동</h2>
<p>앱은 이름·연락처·생년월일을 받지 않습니다. 로그인과 구독은 Google 계정과 Google Play 를 통해 이루어지며, 만 14세 미만 아동의 계정은 Google 정책에 따라 보호자가 관리합니다. 법정대리인은 6번의 권리를 아동을 대신해 행사할 수 있습니다.</p>
